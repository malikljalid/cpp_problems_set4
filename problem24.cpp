...
stDate increaseDateByOneMonth(stDate Date)
{
    ...
    {
        if (isLastDayInMonth(Date.Day, Date.Month, Date.Year) && getMonthDays(Date.Month, Date.Year) > getMonthDays(Date.Month + 1, Date.Year))
...
stDate increaseDateByXMonths(stDate Date, short int MonthsToAdd)
{
    while (MonthsToAdd--)
